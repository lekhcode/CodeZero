...
if (!parser.includes("extractExamplesFromHtml")) {
  const idx = parser.indexOf("export function parseExamples");
  let tail = parser.slice(idx);
  tail = tail.replace(/^\/\*\*[\s\S]*?Build structured examples from LeetCode fields\.[\s\S]*?\*\/\s*/m, "/** Build structured examples from LeetCode GraphQL testcase fields (judge-oriented; may lack outputs). */\n");
  parser = parser.slice(0, idx) + insert + tail;
...
service = service.replace(
  "hasStoredDetail(cached) && hasStoredExamples(cached)",
  "hasStoredDetail(cached) && hasCompleteStoredExamples(cached)",
);
